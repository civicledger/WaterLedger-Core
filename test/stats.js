...
contract('Stats', function(accounts) {
  ...
  it("should allow the volume to be set", async function() {
    ...
    let volumeObj = await statInstance.volumeAvailable();
  ...
  it("should reject setting the volume if user is incorrect", async function() {
    ...
    assert.isTrue(exceptionThrown, 'Exception was not thrown');
    assert.equal(reason, "Only owner can set volume", 'Wrong revert reason');
  });

  it("should allow a user to set all of the stats", async () => {
    await statInstance.setStats(1, 2, 3, 4, 5);
    let minBid = await statInstance.yesterdayMinBid();
    let maxBid = await statInstance.yesterdayMaxBid();
  ...
  xit("should trigger VolumeAvailableChanged event", async function() {
    await statInstance.setVolumeAvailable(defaultVolume);

    let volumeObj = await statInstance.volumeAvailable();
    assert.equal(volumeObj.words[0], defaultVolume, 'Volume Available is not set correctly');
  });

  xit("should reject setting the volume if not owner", async function() {
    await statInstance.setVolumeAvailable(defaultVolume);
